...
if __name__ == '__main__':
    ...
    parser.add_argument('--seq_len_eval', type=int, default=1024)
    ...
    parser.add_argument("--seq_len_scaler", '-ss', help='seq_len_scaler', type=float, default=None)
    parser.add_argument("--scaling_type", '-st', help='scaling type', type=str, default=None)
    ...
    parser.add_argument("--drop_prob", help='dropout', type=float, default=0.0)
    ...
    mamba_cfg = {'mamba_name': args.mamba, 'hidden_size': args.d_model, 'num_hidden_layers': args.n_layers,
                 'state_size': args.d_state, 'vocab_size': 0, 'conv_kernel': 4, 'drop_prob': args.drop_prob}
    ...
    model_cfg = {'model_name': 'FoveatedMamba6', 'mamba_cfg': mamba_cfg, 'pos_emb_type': args.pos_emb_type,
    ...
    train_args_cfg = {'seq_len_range': tuple(args.seq_len_range), 'seq_len_eval': args.seq_len_eval,
                      'clip_grad': args.clip, 'intermediate_save': args.intermediate_save, 'query_len': args.query_len,
                      'seq_len_scaler': args.seq_len_scaler, 'scaling_type': args.scaling_type}
    ...
    if args.with_id is not None:
        ...
        # checkpoint = nu.get_checkpoint(args.init_from)
        # cfg = nu.get_cfg(args.init_from)
        cfg['model_cfg'] = tuples_to_strings(nu.get_cfg(args.init_from)['model_cfg'])
        run[strings.CFG] = cfg
        model_state = nu.get_model_state(args.init_from, update=True)
    else:
        run[strings.CFG] = cfg
