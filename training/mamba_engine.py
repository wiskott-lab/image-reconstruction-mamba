...
import config
from training.utils import eval_seq_img_mamba
...
def train(model, epochs, dataloader_train, dataloader_val, optimizer, loss_scaler, val_step, train_step, best_loss,
          lr_scheduler, run, seq_len_range, seq_len_eval, query_len, clip_grad=1.0, intermediate_save=None, seq_len_scaler=None, scaling_type=None):
    model_state = generate_model_state(model)
    if best_loss is None:
        best_loss = eval_seq_img_mamba(model=model, dataloader=dataloader_val, run=run, seq_len=seq_len_eval, query_len=query_len)
    ...
    for epoch in range(epochs):
        ...
        for batch_id, (img, target) in enumerate(dataloader_train):
            ...
            query_seq, query_pos_x, query_pos_y = patcher.get_seq(fov_props=model.foveation_properties, img=img,
                                                                  seq_len=query_len,
                                                                  seq_type='rnd')
            recon = model.forward_seq(seq=seq, pos_x=pos_x, pos_y=pos_y, query_pos_x=query_pos_x,
                                      query_pos_y=query_pos_y)
            optimizer.zero_grad()
            loss = F.mse_loss(input=dataloader_val.dataset.transform.transforms[-1](recon), target=query_seq[0])
            if seq_len_scaler is not None:
                if scaling_type is None:
                    scaled_loss = loss / (seq[0].shape[1] ** seq_len_scaler)
                if scaling_type == 'xtox':
                    scaled_loss = loss / (seq[0].shape[1] ** (seq[0].shape[1] ** seq_len_scaler))
                loss_scaler(scaled_loss, optimizer, clip_grad=clip_grad, parameters=model.parameters())
            else:
                loss_scaler(loss, optimizer, clip_grad=clip_grad, parameters=model.parameters())
            # loss.backward()
            train_step += 1
            run["series/train/loss"].append(loss.detach().item())
        val_loss = eval_seq_img_mamba(model=model, dataloader=dataloader_val, run=run, seq_len=seq_len_eval,  query_len=query_len)
        val_step += 1
