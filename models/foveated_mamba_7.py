...
class FoveatedMamba7(nn.Module):
    ...
    def __init__(self, mamba, heads: nn.ModuleList, action_converter, backbones: nn.ModuleList, joiner,
        ...
        self.length_embeder = nn.Linear(1, mamba.config.hidden_size)
    ...
    def forward_seq(self, seq, pos_x, pos_y, query_pos_x, query_pos_y, weird_forward=False, cache_params=None, time_scalers=None, seq_length=None):
        ...
        if seq_length is None:
            seq_length = img_seq.shape[1]
        length_token = self.length_embeder(torch.tensor(seq_length / 100).unsqueeze(0).to(config.DEVICE)).expand(img_seq.shape[0], -1).unsqueeze(1)
        ...
        if weird_forward:
            ...
            seq = torch.concat([length_token, img_seq, delimiter_token, query_token], dim=1)
            y = self.mamba(inputs_embeds=seq, use_cache=False, cache_params=None, time_scalers=time_scalers)
            outputs = self.forward_through_head(y.last_hidden_state[:, img_seq.shape[1] + 2:])
            return outputs
